...
export function Avatar(props) {
  ...
  const { animations } = useGLTF("/models/myAnimations.glb");

  const group = useRef();
  const { actions, mixer } = useAnimations(animations, group);
  const [animation, setAnimation] = useState(() => {
    const idleAnimation = animations.find(a => a.name === "Idle");
    return idleAnimation ? "Idle" : (animations.length > 0 ? animations[0].name : null);
  });

  useEffect(() => {

    // Add a safety check for the animation action
    if (!actions || !actions[animation]) return;
    const action = actions[animation];

    // Reset and play only if the action exists
    action.reset().fadeIn(mixer.stats.actions.inUse === 0 ? 0 : 0.5).play();

    return () => {
      // Add fadeOut only if action exists
      action.fadeOut(0.5);
    };
  ...
  return (
    <group {...props} ispose={null} ref={group}>
...
useGLTF.preload("/models/myAnimations.glb");