...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Pause/resume animation on visibility  //! Default animation
    ...
    emojis.addEventListener('click', (event) => {
        ...
        // Check if the clicked element is one of the emojis
        if (target.classList.contains('emoji')) {
            const emoji = target.textContent.trim();

            // Clear fixed values that might interfere with emoji animations
            // Store current morph values so we can restore them later
            const currentMorphValues = {};
            head.getMorphTargetNames().forEach((mt) => {
                const el = d3.select('#' + CSS.escape(mt));
                if (!el.empty()) {
                    currentMorphValues[mt] = parseFloat(el.property("value"));
                    // Temporarily clear fixed value for facial expressions
                    if (!mt.startsWith('headRotate') && !mt.startsWith('bodyRotate') && !mt.startsWith('eyesRotate')) {
                        head.setFixedValue(mt, null);
                    }
                }
            });

            // Play the emoji animation
            head.speakEmoji(emoji);

            // Optional: Restore morph values after a delay (adjust timing as needed)
            // This allows the emoji to play, then returns to your custom morph settings
            setTimeout(() => {
                head.getMorphTargetNames().forEach((mt) => {
                    const el = d3.select('#' + CSS.escape(mt));
                    if (!el.empty() && currentMorphValues[mt] !== undefined) {
                        head.setFixedValue(mt, testerGetFixedValue(mt, currentMorphValues[mt]));
                    }
                });
            }, 2000);
        }
    ...
    //!/Morph Targets Controler///
    const reset = document.getElementById('reset');
    reset.addEventListener('click', () => {
        // Loop over all morph targets
        head.getMorphTargetNames().forEach((mt) => {
            const el = d3.select('#' + CSS.escape(mt));
            if (!el.empty()) {
                // Compute reset value
                const resetValue = testerGetValue(mt, null); // usually 0 or -0.1
                el.property('value', resetValue);            // reset slider
                d3.select(el.node().nextElementSibling)
                    .html(testerGetLabel(mt, resetValue));      // reset displayed label
                head.setFixedValue(mt, testerGetFixedValue(mt, resetValue)); // reset on model
            }
        });

        testerUpdateMorphData(); // optional: update any displayed JSON or output
    });
    ...
    function testerGetFixedValue(mt, v) {
        // For rotation shapes, return as-is
        if (mt.startsWith('headRotate') || mt.startsWith('bodyRotate') || mt.startsWith('eyesRotate')) {
            return v;
        }
        // Otherwise, clip the value to [0,1] so negative values like -0.1 don't apply
        return Math.max(0, v);
    }
    ...
    // Alternative approach: Add a "Clear Fixed Values" button for emoji mode
    function enableEmojiMode() {
        head.getMorphTargetNames().forEach((mt) => {
            // Clear fixed values for facial expressions only, keep head/body rotation
            if (!mt.startsWith('headRotate') && !mt.startsWith('bodyRotate') && !mt.startsWith('eyesRotate')) {
                head.setFixedValue(mt, null);
            }
        });
    ...
    // You can call this before using emojis if you want manual control
    // enableEmojiMode();
